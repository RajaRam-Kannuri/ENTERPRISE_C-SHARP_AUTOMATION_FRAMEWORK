...
namespace PLI.Automation.Pages
{
    public abstract class ApplicationAdultInformationPage : ApplicationWizardPage
    {
        ...
        public ApplicationAdultInformationPage(IWebDriver driver)
            : base(driver)
        {
        ...
        public void EnterAdultData(AdultData parentData)
        {
            WaitForPageLoad();
            WebElement radioChoice;
            FirstName.SetText(parentData.FirstName);
            MiddleName.SetText(parentData.MiddleName);
            LastName.SetText(parentData.LastName);
            EmailAddress.SetText(parentData.EmailAddress);
            HardWait(3);

            if (parentData.EmploymentStatus != null && parentData.EmploymentStatus == "Employed")
            {
                EmploymentStatus.Select(parentData.EmploymentStatus);
                WaitForElement(Employer);
                Employer.SetText(parentData.Employer);
            }

            radioChoice = parentData.IncomeChanges == "Yes" ? IncomeChangesYes : IncomeChangesNo;
            radioChoice.Set(true);

            radioChoice = parentData.ReceivingChildSupport == "Yes" ? ReceivingChildSupportYes : ReceivingChildSupportNo;
            radioChoice.Set(true);

            CheckWages.SetText(parentData.CheckWages);
            CashWages.SetText(parentData.CashWages);
            NonWorkCompensation.SetText(parentData.NonWorkCompensation);
            ChildSupport.SetText(parentData.ChildSupport);
            Alimony.SetText(parentData.Alimony);
            AdoptionBenefits.SetText(parentData.AdoptionBenefits);
            PublicAssistance.SetText(parentData.PublicAssistance);
            AdultSocialSecurity.SetText(parentData.AdultSocialSecurity);
            ChildSocialSecurity.SetText(parentData.ChildSocialSecurity);
            SupplementalSecurity.SetText(parentData.SupplementalSecurity);
            Pension.SetText(parentData.Pension);
            AllowanceBenefits.SetText(parentData.AllowanceBenefits);
            DeployedServiceMember.SetText(parentData.DeployedServiceMember);
            Annuities.SetText(parentData.Annuities);
            Interest.SetText(parentData.Interest);
            MonthlyWithdrawals.SetText(parentData.MonthlyWithdrawals);
            OutsideAssistance.SetText(parentData.OutsideAssistance);
            SelfEmployment.SetText(parentData.SelfEmployment);
            NetRental.SetText(parentData.NetRental);
            OtherIncome.SetText(parentData.OtherIncome);
            FosterOOHC.SetText(parentData.FosterOOHC);
        }

        public void SubmitOtherAdultData(AdultData parentData)
        {
            EnterAdultData(parentData);
            Next();
            WaitForPageLoad();
        }
    }
